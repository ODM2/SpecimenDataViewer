...
def get_sampling_feature_info(request):
    if request.method == 'GET':
        ...
        if sampling_feature_type and not sfids:
            ...
            sampling_features = SamplingFeature.objects.filter(pk__in=sfids_list)
        else:
            sampling_features = SamplingFeature.objects.all()[:100]

        serialized_data = serializers.SamplingFeatureInfo('json', sampling_features, many=True)
...
def get_dataset_metadata(request):
    if request.method == 'GET':
        ...
        data_set_type = request.GET.getlist('datasettype') if 'datasettype' in request.GET else None
